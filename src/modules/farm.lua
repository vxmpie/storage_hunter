	...
	function FarmModule.startAuctionAndCollect(zoneName)
		...
		if targetPrompt and targetGarage then
			...
			while auctionUI and not auctionUI.Visible and uiWait < 8 and Config.IsFarming do
				task.wait(0.5); uiWait = uiWait + 0.5
			end
			
			if auctionUI and auctionUI.Visible then
				local noneVisibleCount = 0
				while Config.IsFarming do
					if auctionUI.Visible then noneVisibleCount = 0 else noneVisibleCount = noneVisibleCount + 1 end
				...
				if Config.AutoUnload then
					...
					if currentVehicle then
						...
						if cargoWeight >= (cargoLimit * 0.95) then
							WashModule.warpToUnpack()
							task.wait(1)
							if currentVehicle then currentVehicle:PivotTo(savedGarageCFrame) else Utils.warpTo(savedGarageCFrame) end
							task.wait(0.5)
						end
					end
				end
				...
				for _, item in pairs(carryables:GetChildren()) do
					...
					if prompt and prompt.Enabled and basePart then
						...
						if dist <= 60 then
							...
							if weightValue and (weightValue:IsA("NumberValue") or weightValue:IsA("IntValue")) then weight = weightValue.Value
							else weight = tonumber(item:GetAttribute("Weight")) or tonumber(item:GetAttribute("Mass")) or 0 end
